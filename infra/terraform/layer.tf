...
data "archive_file" "zip_imaging_layer" {
  type        = "zip"
  source_dir  = "./api/layer-imaging"
  output_path = "./api/imaging_layer.zip"
}

resource "aws_lambda_layer_version" "imaging" {
  layer_name          = "api_imaging"
  filename            = data.archive_file.zip_imaging_layer.output_path
